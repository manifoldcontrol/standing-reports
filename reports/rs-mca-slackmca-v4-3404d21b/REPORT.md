...
Reed-Solomon codes on smooth multiplicative domains. The repository's log
records the formalization as drafted by one AI system (Aristotle/Harmonic)
and reviewed and packaged by another (Codex).

## verdict

Standing: **release_verified** for the Part I entropy claim (locator
fibers, coefficient pigeonhole, quotient cores, inverse quotient, fiber
and entropy bounds) and the Part II rigidity claim (cyclotomic and
...
This reproduces, from the kernel, the repository's statement that its
main theorems depend only on the standard axioms.
...
Lean source, definitional roots first. It verifies that the formalized
decoding list, monomial-prefix words, and bad-parameter predicate are the
paper's objects, and that every divergence found is a generalization.
Five residuals are named in the attestation; none is required by the two
claims.
...
- `external_validation_boundary_open`: no human expert review of this
  package is on record, and the corresponding claim's standing is null.
